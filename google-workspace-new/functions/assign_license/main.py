...
def assign_license_handler(request: Request):
    try:
        ...
        assign_license(user_email, product_id, license_sku, license_service)

        return make_response(
            f"License, {product_id}: {license_sku} assigned to user, "
        ...
        return make_response(
            f"Failed to assign license to user. Error: {e}", 400
        )
...
def assign_license(user_email, product_id, license_sku, service):
    request_body = {
        "userId": user_email,
    }
    service.licenseAssignments().insert(
        productId=product_id, skuId=license_sku, body=request_body
    ).execute()
