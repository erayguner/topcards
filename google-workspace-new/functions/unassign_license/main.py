...
def unassign_license_handler(request: Request):
    try:
        user_email = request.json.get("user_email")
        product_id = request.json.get("product_id")
        license_sku = request.json.get("license_sku")
        creds = Credentials()
        directory_service = creds.build("admin", "directory_v1")
        license_service = creds.build("licensing", "v1")

        user = get_user(user_email, directory_service)
        if not user:
            raise ValueError(f"User, {user_email} does not exist.")

        unassign_license(user_email, product_id, license_sku, license_service)

        return make_response(
            f"License, {product_id}: {license_sku} unassigned from user, "
            f"{user_email} successfully.", 200
        )
    except KeyError as ke:
        return make_response(f"Missing parameter: {ke}", 400)
    except Exception as e:
        return make_response(
            f"Failed to unassign license from user. Error: {e}", 400
        )
...
def unassign_license(user_email, product_id, license_sku, service):
    service.licenseAssignments().delete(
        productId=product_id, skuId=license_sku, userId=user_email
    ).execute()
