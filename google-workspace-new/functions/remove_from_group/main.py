...
def remove_from_group_handler(request: Request):
    try:
        ...
        if not is_user_in_group(user_email, group_email, service):
            return make_response(
                f"User, {user_email} is not in group, {group_email}.", 200
            )

        remove_from_group(user_email, group_email, service)

        return make_response(
            f"User removed from group, {group_email} successfully.", 200
        )
    except KeyError as ke:
        return make_response(f"Missing parameter: {ke}", 400)
    except Exception as e:
        return make_response(
            f"Failed to remove user from group. Error: {e}", 400
        )
...
def remove_from_group(user_email, group_email, service):
